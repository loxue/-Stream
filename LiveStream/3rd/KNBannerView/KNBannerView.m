...
//    bannerView.locationImgArr = locationImgArr;
    return bannerView;
}
...
- (void)initializeCollectionView{
    ...
    NSInteger _bannerViewHeight = (SCREEN_WIDTH==320)?(150):(SCREEN_WIDTH == 375?(160):170);
    layout.itemSize = CGSizeMake(SCREEN_WIDTH, _bannerViewHeight); // set layout flow size
    ...
    UICollectionView *collectView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH-30,_bannerViewHeight) collectionViewLayout:layout];
    
    [collectView setBackgroundColor:[UIColor clearColor]];
    
//    collectView.pagingEnabled = (_IMGArray.count<=1)?NO:YES;
...
- (void)initDefaultData{
    ...
    _IntroduceStyle = KNIntroduceStyleRight;
    _IntroduceHeight = 30;
    _timeInterval = 1.5;
...
- (void)setupPageControl{
    ...
    BAPageControl *pageControl = [[BAPageControl alloc] initWithFrame:CGRectZero indicatorMargin:5.f indicatorWidth:10.f currentIndicatorWidth:15.f indicatorHeight:5.f];
...
- (void)setCurrentPageIndicatorTintColor:(UIColor *)CurrentPageIndicatorTintColor{
    ...
    _pageControl.currentPageIndicatorColor = CurrentPageIndicatorTintColor;
}

- (void)setPageIndicatorTintColor:(UIColor *)PageIndicatorTintColor{
    _PageIndicatorTintColor = PageIndicatorTintColor;
    _pageControl.pageIndicatorColor = PageIndicatorTintColor;
}
...
//        [_collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:_imagesCount * 0.5 inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
    ...
    switch (_pageControlStyle) {
        case KNPageControlStyleMiddle:
            _pageControl.frame = RECT(0, _bannerViewHeight - 30 , SCREEN_WIDTH, 30);
            break;
        case KNPageControlStyleLeft:
            _pageControl.frame = RECT(10 - self.width * 0.5 + _IMGArray.count * 0.5 * 10 + (_IMGArray.count - 1)  * 0.5 * 5, _bannerViewHeight - 30, self.width, 30);
            break;
        case KNPageControlStyleRight:
//            _pageControl.frame = CGRectMake(self.width * 0.5 - ((_IMGArray.count) * 10 + (_IMGArray.count - 1) * 5) * 0.5 - 10, _bannerViewHeight - 30 ,self.width, 30);
            _pageControl.frame = RECT(SCREEN_WIDTH-80, _bannerViewHeight-40, 100, 20);
            break;
        default:
            break;
    }
