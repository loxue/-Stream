...
#import "BABaseRequestManger.h"

@implementation BABaseRequestManger
...
static BABaseRequestManger *_instance = nil;
...
// post
- (void)POST:(NSString *)urlString parameters:(id)parameters completion:(void (^)(BABaseNetResponse *))completion {
    [self request:@"POST" URL:urlString parameters:parameters completion:completion];
}

//get
- (void)GET:(NSString *)urlString parameters:(id)parameters completion:(void (^)(BABaseNetResponse *))completion {
...
- (void)request:(NSString *)method URL:(NSString *)urlString parameters:(id)parameters completion:(void (^)(BABaseNetResponse *response))completion {
    ...
    if (self.reachabilityManager.networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
        BABaseNetResponse *response = [BABaseNetResponse new];
...
#pragma mark - 加载本地数据
- (void)requestLocal:(NSString *)urlString completion:(void (^)(BABaseNetResponse *response))completion {
...
#pragma mark - 处理数据
- (void)wrapperTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error completion:(void (^)(BABaseNetResponse *response))completion {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        BABaseNetResponse *response = [self convertTask:task responseObject:responseObject error:error];
...
#pragma mark - 包装返回的数据
- (BABaseNetResponse *)convertTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error {
...
#pragma mark - 打印返回日志
- (void)LogResponse:(NSString *)urlString response:(BABaseNetResponse *)response {
...
- (void)configSettings {
    ...
    //自定义处理数据
    self.responseFormat = ^BABaseNetResponse *(BABaseNetResponse *response) {
        return response;
    };
}
